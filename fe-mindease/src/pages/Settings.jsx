...
export default function Settings() {
  ...
  const handleFileChange = async (e) => {
    ...
    setMessage('');
    try {
      const res = await fetch(`${API_URL}/auth/profile/upload`, {
        method: 'POST',
      ...
      if (res.ok) {
        const data = await res.json();
        setFormData(prev => ({ ...prev, profile_picture: data.profile_picture }));
        setMessage('Foto profil berhasil diunggah! Silakan Simpan Perubahan.');
      } else {
        const err = await res.json();
        setMessage(`Gagal mengunggah foto: ${err.error}`);
      }
    } catch (err) {
      setMessage('Terjadi kesalahan saat mengunggah foto.');
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              {formData.profile_picture ? (
                <img src={formData.profile_picture} alt="Profile" className="w-16 h-16 rounded-full object-cover border" style={{ borderColor: 'var(--border)' }} />
              ) : (
                <div className="w-16 h-16 rounded-full bg-emerald-500 text-white flex items-center justify-center font-bold text-2xl">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs mt-1" style={{ color: 'var(--t-muted)' }}>Pilih foto dari perangkat Anda</p>
              </div>
            </div>
          </div>
